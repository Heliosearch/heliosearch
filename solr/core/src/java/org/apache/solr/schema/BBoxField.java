  ...
  protected ValueSource getValueSourceFromSpatialArgs(QParser parser, SchemaField field, SpatialArgs spatialArgs, String scoreParam, BBoxStrategy strategy) {
    switch (scoreParam) {
      ...
      case "overlapRatio":
        ...
        return new ValueSourceAdapter(new BBoxOverlapRatioValueSource(
            strategy.makeShapeValueSource(), ctx.isGeo(),
            (Rectangle) spatialArgs.getShape(),
            queryTargetProportion, minSideLength));

      case "area":
        return new ValueSourceAdapter(new ShapeAreaValueSource(strategy.makeShapeValueSource(), ctx, ctx.isGeo()));

      case "area2D":
        return new ValueSourceAdapter(new ShapeAreaValueSource(strategy.makeShapeValueSource(), ctx, false));
