...
//      return new TermOrdValComparator(context, numHits, missingLast);
      return new NativeStrComparator(context, numHits, missingLast);
    }
  }



  public class NativeStrComparator extends FieldComparator<BytesRef> {
    ...
    private final long[] termPointer;
    final int[] readerGen;
    ...
    public NativeStrComparator(QueryContext qcontext, int numHits, boolean missingLast) {
      this.qcontext = qcontext;
      this.topValues = (StrTopValues) getTopValues(qcontext);

      ords = new int[numHits];
      termPointer = new long[numHits];
    ...
    public int compare(int slot1, int slot2) {
      ...
      long termPointer1 = termPointer[slot1];
      long termPointer2 = termPointer[slot2];
      ...
      } else if (termPointer2 == 0) {
        return -missingSortCmp;
      }

      return HS.compareLengthPrefixBytes(termPointer1, termPointer2);
    }
    ...
    public void copy(int slot, int doc) {
      ...
      if (ord < 0) {
        ...
        termPointer[slot] = strValues.ordToTermPointer(ord);
      }
      ords[slot] = ord;
    ...
    public void setBottom(final int bottom) {
      ...
      bottomTermPointer = termPointer[bottomSlot];
      if (currentReaderGen == readerGen[bottomSlot]) {
        bottomOrd = ords[bottomSlot];
        bottomSameReader = true;
      } else {
        if (bottomTermPointer == 0) {
          ...
          final int ord = (int)strValues.termPointerToOrd(bottomTermPointer);
          if (ord < 0) {
            bottomOrd = -ord - 2;
