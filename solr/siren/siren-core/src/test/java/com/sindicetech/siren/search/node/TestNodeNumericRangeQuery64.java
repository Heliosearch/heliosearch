...
public class TestNodeNumericRangeQuery64 extends SirenTestCase {
  ...
  private void testRandomTrieAndClassicRangeQuery(final int precisionStep) throws Exception {
    ...
    for (int i = 0; i < num; i++) {
      ...
      lowerBytes.append(new BytesRef(FieldType.NumericType.LONG.toString() + precisionStep));
      final BytesRefBuilder upperBytes = new BytesRefBuilder();
      upperBytes.append(new BytesRef(FieldType.NumericType.LONG.toString() + precisionStep));
