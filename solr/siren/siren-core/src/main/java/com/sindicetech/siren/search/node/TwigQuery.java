...
import com.sindicetech.siren.search.node.NodeBooleanQuery.AbstractNodeBooleanWeight;
import com.sindicetech.siren.search.node.NodeBooleanQuery.TooManyClauses;
import com.sindicetech.siren.search.node.TwigQuery.EmptyRootQuery.EmptyRootWeight.EmptyRootScorer;
...
public class TwigQuery extends NodeQuery {
  ...
  */
  ...
  public void setLevelConstraint(final int levelConstraint) {
    // Do not update the level constraint if it is the sentinel value - cf. #19
  ...
  protected class TwigWeight extends AbstractNodeBooleanWeight {
    ...
    throws IOException {
      final NodeScorer rootScorer = (NodeScorer) rootWeight.scorer(context, acceptDocs);
      ...
      for (final Weight w  : weights) {
        final NodeBooleanClause c =  cIter.next();
        final NodeScorer subScorer = (NodeScorer) w.scorer(context, acceptDocs);
        if (subScorer == null) {
          if (c.isRequired()) {
            return null;
          }
